...
(function(){
 ...
 function buildRemote(){
   ...
   // --- y2: max-to-mean skew (ratio, dimensionless) OR max−min spread (delta, bytes) —
   ...
   if(y2mode){
     ...
     if(rmx>0){
       ...
       s.push('<text class="axtitle" transform="rotate(-90 '+(x1+30)+' '+((yt+yb)/2)+')" x="'+(x1+30)+'" y="'+((yt+yb)/2)+'" text-anchor="middle">'+(y2mode==='Delta'?('max−min '+runit+' by node'):'max-to-mean skew')+'</text>');
